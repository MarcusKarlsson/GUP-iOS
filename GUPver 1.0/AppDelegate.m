...
@synthesize hasInet;
@synthesize session = _session, groupViewController, firstViewController, explorViewController, shareViewController, settingViewController, rootViewControllerL;
...
- (void)applicationDidBecomeActive:(UIApplication *)application {
    ...
    // Rating GUP
    NSString* rateGUP = [[NSUserDefaults standardUserDefaults] objectForKey:@"RATE_GUP"];
    
    if (rateGUP == nil || (![rateGUP isEqualToString:@"CANCELED"] && ![rateGUP isEqualToString:@"RATED"])) {
        int launchCount = 0;
        if (rateGUP) {
            launchCount = [rateGUP intValue];
        }
        if (launchCount % 5 == 4) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Do you enjoy using GUP?"
                                                            message:@"If you like this app and find it useful, could you please take a moment to rate it? Your feedback will be highly appreciated."
                                                           delegate:self
                                                  cancelButtonTitle:nil
                                                  otherButtonTitles:@"Remind Me Later", @"No Thanks", @"Rate It Now", nil];
            [alert setTag: 200];
            [alert show];
        }
        NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:[NSString stringWithFormat:@"%d", launchCount + 1] forKey:@"RATE_GUP"];
        [defaults synchronize];
    }
    NSLog(@"===============");
    NSLog(@"=== AppDelegate didBecomeActive Ended.");
    NSLog(@"===============");
...
-(void)freezerAnimate {
    ...
    UINavigationController* rootView = (UINavigationController*)[[self.tabBarController viewControllers] objectAtIndex:tabIndex];
    UIViewController* currentView = [rootView topViewController];
    NSLog(@"%@", currentView.debugDescription);
    if (currentView
        && ![currentView isKindOfClass:[PostListing class]]
        && ![currentView isKindOfClass:[CommentViewController class]]
        && ![currentView isKindOfClass:[ChatScreen class]])
    {
        if ([currentView isKindOfClass:[GroupViewController class]]) {
            [(GroupViewController*)currentView refreshGroupList];
        } else if ([currentView isKindOfClass:[FirstViewController class]]) {
            [(FirstViewController*)currentView refreshChatList];
        }
        HUD = [MBProgressHUD showHUDAddedTo:currentView.view animated:YES];
        HUD.dimBackground = YES;
        HUD.labelText = @"Please Wait";
        HUD.hidden = NO;
        [NSTimer scheduledTimerWithTimeInterval:3.0 target:self selector:@selector(freezerRemove) userInfo:nil repeats:NO];
    }
}
...
-(void)setTabBar{
    ...
    groupViewController = [[GroupViewController alloc] initWithNibName:@"GroupViewController_iPhone" bundle:nil];
    explorViewController = [[ExploreViewController alloc] initWithNibName:@"ExploreViewController" bundle:nil];
    ...
    UINavigationController *navigationController1 = [[UINavigationController alloc] initWithRootViewController:groupViewController];
    UINavigationController *navigationController2 = [[UINavigationController alloc] initWithRootViewController:firstViewController];
    UINavigationController *navigationController3 = [[UINavigationController alloc] initWithRootViewController:explorViewController];
    UINavigationController *navigationController4 = [[UINavigationController alloc] initWithRootViewController:shareViewController];
    UINavigationController *navigationController5 = [[UINavigationController alloc] initWithRootViewController:settingViewController];
    ...
    imgTab = [[UIImageView alloc]initWithFrame:CGRectMake(0,0,self.firstViewController.view.bounds.size.width,50)];
    [self.tabBarController.tabBar addSubview:imgTab];
    self.tabBarController.delegate = self;
    
    float width1= ([[UIScreen mainScreen] bounds].size.width - 150)/6;
    float width = 30;
...
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult result))handler{
    ...
    if (dictionary!=nil){
        ...
        NSLog(@"=== notificationType %d", notificationType);
        NSLog(@"===============");
...
-(void)get_UTC_Time
{
    if ( getUTCtime==nil)
    {
        ...
        if (currentUser&&_messageDelegate!=nil)
        {
            [_messageDelegate freezerAnimate];
        }
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
        
        NSString *url= [NSString stringWithFormat:@"%@/scripts/utc_time.php", gupappUrl];
...
-(void) CurrentDate{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        NSURL * scriptUrl = [NSURL URLWithString: @"http://gupapp.com/Gup_demo/scripts/utc_time.php"];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (data != nil)
            {
                ...
                NSLog(@"===============");
                NSLog(@"=== AppDelegate CurrentDate dispatch_async_ended");
                NSLog(@"===============");
            }
            else {
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [_locationManager startUpdatingLocation];
    // Initialize rating
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"RATE_GUP"];
...
-(BOOL)PutLinkOfMessageInStorageForType:(NSString*)form withMessageData:(NSDictionary*)messageComponents{
    NSLog(@"===============");
    NSLog(@"=== AppDelegate");
    NSLog(@"=== PutLinkOfMessageInStorageForType: messageComponents");
    NSLog(@"===============");
...
//                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (![[from userID]isEqualToString:myUserID]&&![self CheckIfMessageIsDuplicateFrom:[NSString stringWithFormat:@"%@",[from userID]] ofMessageTime:timestamp isGroupMsg:isGroup]){
                        ...
                        if ([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive ){
                            if (currentUser && [[currentUser userID] isEqualToString:[from userID]]){
                            ...
                            if (currentUser && [[currentUser userID] isEqualToString:from])
                                [_messageDelegate newMessageReceived ];
                            else {
                                NSLog(@"=== 5");
                                NSLog(@"===============");
        ...
        @try {
            ...
            if([gup elementForName:@"newpostnotification"]){
                ...
                if([userid intValue] != [self.myUserID intValue]){
                    BOOL isNewPost = NO;
                    NSString* groupid = [[gup elementForName:@"groupid"] stringValue];
                    NSString* postid = [[gup elementForName:@"postid"] stringValue];
                    NSString *timeInMiliseconds =[[NSString getCurrentUTCFormateDate] getTimeIntervalFromUTCStringDate];
                    
                    NSArray *private =  [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:
                                         [NSString stringWithFormat:@"SELECT latest_post_id from groups_private WHERE group_server_id = %@",groupid]];
                    NSArray *public =  [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:
                                        [NSString stringWithFormat:@"SELECT latest_post_id from groups_public WHERE group_server_id = %@",groupid]];
                    if (private.count > 0) {
                        NSString* latest = [NSString stringWithFormat:@"%@",[[private objectAtIndex:0] objectForKey: @"LATEST_POST_ID"]];
                        if ([latest isEqualToString:@"<null>"] || [latest intValue] < [postid intValue]) {
                            [[DatabaseManager getSharedInstance]executeQueryWithQuery:[NSString stringWithFormat:
                                                                                       @"UPDATE groups_private SET new_post = 1, latest_post_id=%@, updatetime=%@ WHERE group_server_id=%@",
                                                                                       postid, timeInMiliseconds, groupid]];
                            isNewPost = YES;
                        }
                    } else if (public.count > 0) {
                        NSString* latest = [NSString stringWithFormat:@"%@",[[public objectAtIndex:0] objectForKey: @"LATEST_POST_ID"]];
                        if ([latest isEqualToString:@"<null>"] || [latest intValue] < [postid intValue]) {
                            [[DatabaseManager getSharedInstance]executeQueryWithQuery:[NSString stringWithFormat:
                                                                                       @"UPDATE groups_public SET new_post = 1, latest_post_id=%@, updatetime=%@ WHERE group_server_id=%@",
                                                                                       postid, timeInMiliseconds, groupid]];
                            isNewPost = YES;
                        }
                    }
                    if (isNewPost) {
                        NSLog(@"=== newPostNotification");
                        NSLog(@"===============");
                        if ([[UIApplication sharedApplication] applicationState] != UIApplicationStateActive){
             ...
             if ([userDetails count]==0 ){
                 ...
                 dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
                     ...
                     dispatch_async(dispatch_get_main_queue(), ^{
                         ...
                         NSLog(@"===============");
                         NSLog(@"=== AppDelegate");
                         NSLog(@"=== getContactDataWithNotificationForUserId");
                         NSLog(@"===============");
                         [_chatDelegate buddyStatusUpdated];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if (connection== getUTCtime)
    {
        ...
        if (  !currentUser&&_messageDelegate!=nil&&previousTimeDifferance!=0.0000)
        {
            [_messageDelegate freezerRemove];
        }
    }
    ...
    if (connection == contactDetailConn) {
        ...
        else
        {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    NSLog(@"===============");
                    NSLog(@"=== AppDelegate");
                    NSLog(@"=== connectionDidFinishLoading, contactDetailConnect");
                    NSLog(@"===============");
                    [_chatDelegate buddyStatusUpdated];
        ...
        @try{
            ...
            if (![status isEqualToString:@"1"])
            {
                ...
                else
                {
                    for (NSDictionary *member in members)
                    {
                        ...
                        else
                        {
                            ...
                            if (currentUser && [[currentUser userID] isEqual:groups[@"id"] ])
                ...
                if ([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive )              {
                    if (currentUser && [[currentUser userID] isEqualToString:grouID])
                    {
            ...
            NSLog(@"===============");
            NSLog(@"=== AppDelegate");
            NSLog(@"=== connectionDidFinishLoading, groupInfoConnect");
            NSLog(@"===============");
            [_chatDelegate buddyStatusUpdated];
        ...
        if ([responce[@"status"] isEqualToString:@"1"]){
            UIAlertView *loginError=[[UIAlertView alloc]initWithTitle:Nil message:responce[@"error_message"] delegate:appDelegateObj.firstViewController cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [loginError setTag:55];
            [loginError show];
            
            
        }else if ([responce[@"status"] isEqualToString:@"2"]){
            UIAlertView *loginError=[[UIAlertView alloc]initWithTitle:Nil message:responce[@"error_message"] delegate:appDelegateObj.firstViewController cancelButtonTitle:@"OK" otherButtonTitles:@"Resend E-mail", nil];
            ...
            if (!email_verified){
                UIAlertView *verifyNoti=[[UIAlertView alloc]initWithTitle:Nil message:[NSString stringWithFormat:@"Verification Pending,Verification Link will Expire in %i Days",verify_days] delegate:appDelegateObj.firstViewController cancelButtonTitle:@"OK" otherButtonTitles:@"Resend Email", nil];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (alertView.tag==77){
        ...
        if ([buttonTitle isEqualToString:@"Rate It Now"]) {

            
//            NSURL* url = [NSURL URLWithString: @"https://userpub.itunes.apple.com/WebObjects/MZUserPublishing.woa/wa/addUserReview?type=Purple+Software&id=863991482&mt=8&o=i"];
            NSURL* url = [NSURL URLWithString: @"itms-apps://itunes.apple.com/app/id863991482"];
            if (url) {
                [[UIApplication sharedApplication] openURL:url];
                NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
                [defaults setObject:@"RATED" forKey:@"RATE_GUP"];
                [defaults synchronize];
            }
        } else if ([buttonTitle isEqualToString:@"No Thanks"]){
            
            NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:@"CANCELED" forKey:@"RATE_GUP"];
            [defaults synchronize];
        }
    }
}
...
//                    [_chatDelegate refreshChatList];
                    ...
                    if (currentUser && [currentUser isEqualToString:from] && isUSER){
...
#ifdef DEVELOP_MODE
        location = [[CLLocation alloc] initWithLatitude:51.496487
                                              longitude:-0.461467];
#endif
    return location;
}
...
-(CGFloat)increaseFont {
    if ([self screenWidth] == IPHONE5_SCREEN_WIDTH) {
        return 0;
    } else if ([self screenWidth] == IPHONE6_SCREEN_WIDTH) {
        return 1;
    } else if ([self screenWidth] == IPHONE6P_SCREEN_WIDTH) {
        return 2;
    }
    return 0;
}
