...
//        self.navigationItem.rightBarButtonItem.tintColor = [UIColor blackColor];
    ...
    addButton = [[UIButton alloc] initWithFrame: CGRectMake(0, 0, 60.0f, 30.0f)];
    [addButton setTitle:@"Add" forState:UIControlStateNormal];
    [addButton addTarget:self action:@selector(joinAllGroup) forControlEvents:UIControlEventTouchUpInside];
    [addButton setTitleColor:[UIColor colorWithRed:255.0/255.0 green:207.0/255.0 blue:13.0/255.0 alpha:1] forState:UIControlStateNormal];
    addButton.titleLabel.font = [UIFont fontWithName:@"Dosis-Bold" size:20];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [data setObject:[userId objectAtIndex:indexPath.row] forKey:@"id"];
    ...
    if ([[userId objectAtIndex:indexPath.row] isEqualToString:[[DatabaseManager getSharedInstance] getAppUserID]]) {
        recordExist = YES;
    }
...
-(void)joinAllGroup{
    ...
    [selectedGroup enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        [users addObject:[userId objectAtIndex:row]];
        ...
        if(idx==selectedGroup.count-1){
            ...
            [manager POST:[url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] parameters:data success:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                [selectedGroup enumerateObjectsUsingBlock:^(id objs, NSUInteger idx, BOOL *stop) {
                    ...
                    NSString *insertQuery=[NSString stringWithFormat:@"insert into contacts (user_id, user_email, user_name, user_pic, user_status,user_location) values ('%@', '%@', '%@', '%@', '%@', '%@')", [userId objectAtIndex:rows], @"", [displayName objectAtIndex:rows], [displayPic objectAtIndex:rows], @"", [displayLocation objectAtIndex:rows]];
                    NSLog(@"query %@",insertQuery);
                    [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:insertQuery];
                }];
                
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"The selected users will be available in your profile soon."   delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                [alert show];
...
- (void) openContactProfile:(NSDictionary *)data {
    ...
    viewContact.userLocation = [data objectForKey:@"bottom_display"];
