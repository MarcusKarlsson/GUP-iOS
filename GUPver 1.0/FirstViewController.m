...
- (void)viewDidLoad{
    ...
    if (!ifExists && !ifPublicExists) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:3.0
                                         target:self
                                       selector:@selector(freezerRemove)
                                       userInfo:nil
                                        repeats:NO];
    }
...
- (void)openContactProfile:(NSDictionary *)data {
    ...
    viewContact.userId = [data objectForKey:@"user_id"];
    viewContact.userName = [data objectForKey:@"user_name"];
    viewContact.userImageURL = [data objectForKey:@"user_pic"];
    viewContact.userLocation = [data objectForKey:@"location"];
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [userCell plotCellData:data withLat:0 andLng:0];
}
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if (connection == fetchContactsConn) {
        if ([self isOnTop]) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Unable to fetch contacts from server"
...
-(void)doFetchContacts{
    ...
    if (fetchContactsConn) {
        [fetchContactsConn cancel];
    }
    if (timerCancelFetch) {
        [timerCancelFetch invalidate];
    }
    timerCancelFetch = [NSTimer scheduledTimerWithTimeInterval:15 target:self selector:@selector(cancelFetchContacts) userInfo:nil repeats:NO];
