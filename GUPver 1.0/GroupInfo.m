...
@synthesize groupType,groupId,groupFlog,startLoading,viewType;
...
//        favorite.hidden=true;
    ...
    if([groupType containsString:@"local"]) {
        ...
        [self.locationLbl setTitle:@"Location" forState:(UIControlStateNormal)];
    } else {
...
//            favorite.hidden=true;
        ...
        {            
            ...
            if (getDataPublic.count == 12 && [getDataPublic[10] length] > 0 && [getDataPublic[11] length]) {
                [self.locationLbl setTitle:[NSString stringWithFormat:@"%@, %@", getDataPublic[10], getDataPublic[11]] forState:(UIControlStateNormal)];
            }
...
-(void)plotData{
    ...
    if ([groupType isEqualToString:@"private#local"]||[groupType isEqualToString:@"private#global"] ||[groupType isEqualToString:@"private"]) {
            ...
            if ([getData[1] isEqualToString:@""]) {
                ...
                self.groupDescTextView.text = @"No Description";
            }
            else
            {
                self.groupDescTextView.text  = getData[1];
            }
            ...
            {
                self.groupDescTextView.text  = getDataPublic[1];
            }
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if (connection == leaveGroupConn) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Leave group failure." message:[error localizedDescription]   delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
...
//                        [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:insertMembers];
    ...
    if (connection == leaveGroupConn) {
        ...
        if ([leaveStatus isEqualToString:@"0"])
        {
            ...
            if ([groupFlog isEqualToString:@"1"]) {
                // pending group
                deleteQuery=[NSString stringWithFormat:@"delete from group_invitations where group_id='%@'",groupId];
            } else if ([groupType containsString:@"private"]) {
                // private group
                deleteQuery=[NSString stringWithFormat:@"delete from groups_private where group_server_id='%@'",groupId];
            } else {
                // public group
                deleteQuery=[NSString stringWithFormat:@"delete from groups_public where group_server_id='%@'",groupId];
            }
