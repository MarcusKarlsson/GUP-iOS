...
- (void) fetchLocation
{
    ...
    if (location) {
        ...
        NSString *postData = [NSString stringWithFormat:@"latitude=%f&longitude=%f&type=1",Latitude,Longitude];
...
- (void)viewDidLoad
{
    ...
    [self.mainScroll contentSizeToFit];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if ([buttonTitle isEqualToString:@"Remove Photo"]) {
        ...
        profileImage = nil;
        [UserImage setImage:[UIImage imageNamed:@"createProfile"]];
    }
        ...
        if (validEmailID)
        {
            ...
            if ([passwordTextField.text length]>5&&[passwordTextField.text length]<16) {
                if (usernameIsUnique)
                {
                    ...
                    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
                    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/scripts/add_user_gps.php",gupappUrl]]];
                    [request setHTTPMethod:@"POST"];
                    [request setTimeoutInterval:300];
                    NSMutableData *body = [NSMutableData data];
                    NSString *boundary = @"---------------------------14737809831466499882746641449";
                    ...
                    if (profileImage) {
                        ...
                        [body appendData:[@"Content-Disposition: form-data; name=\"profile_pic\"; filename=\"a.jpg\"\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
                        [body appendData:[@"Content-Type: application/octet-stream\r\n\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
                        imageData = UIImageJPEGRepresentation(profileImage, 0.9);
                        [body appendData:[NSData dataWithData:imageData]];
                        [body appendData:[@"\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
                    }
                    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)sectio{
    ...
    aboutme = [[UITextView alloc] initWithFrame:CGRectMake(10,20, tableView.frame.size.width - 20, 40)];
