...
-(void)viewDidLayoutSubviews {
    ...
    rect = self.categoryInfo.frame;
    rect.size.width = self.view.frame.size.width - self.categoryInfo.frame.origin.x - 30;
    [self.categoryInfo setFrame:rect];
    
    rect = self.totalMembers.frame;
    rect.size.width = self.iconType.frame.origin.x - rect.origin.x - 1;
    [self.totalMembers setFrame:rect];
    
    rect = self.typeInfo.frame;
    rect.origin.x = self.iconType.frame.origin.x + self.iconType.frame.size.width + 10;
    [self.typeInfo setFrame:rect];
    
    rect = self.sharelabel.frame;
    rect.origin.x = self.iconShare.frame.origin.x + self.iconShare.frame.size.width + 10;
    [self.sharelabel setFrame:rect];
}
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    NSLog(@"$[%@]",postData);
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/scripts/group_detail_android_gps.php",gupappUrl]]];
...
-(void)refreshGroupInfo{
    CGFloat addFont = ([[self appDelegate] increaseFont] == 0) ? 0 : 1;
    self.groupNameTextView.text = self.title;
    self.groupNameTextView.textColor=[UIColor colorWithRed:27.0/255.0 green:168.0/255.0 blue:168.0/255.0 alpha:1.0];
    self.groupNameTextView.font = [UIFont fontWithName:@"Dosis-SemiBold" size:18];
    self.groupDescTextView.font = [UIFont fontWithName:@"Dosis-Light" size:13 + addFont];
    self.categoryInfo.font = [UIFont fontWithName:@"Dosis-SemiBold" size:16];
    self.createdInfo.font = [UIFont fontWithName:@"Dosis-Regular" size:11+ addFont];
    ...
    self.locationLbl.titleLabel.font = [UIFont fontWithName:@"Dosis-SemiBold" size:16];
    self.createdInfo.hidden = YES;
    
    self.groupNameTextView.editable = NO;
    self.groupDescTextView.editable = NO;
    self.groupNameTextView.selectable = NO;
    self.groupDescTextView.selectable = NO;
    ...
    if([groupType containsString:@"local"]) {
        self.typeInfo.text =@"Local";
        [self.resetGeoButton setHidden:NO];
        [self.locationImage setHidden:NO];
        [self.locationLbl setHidden:NO];
    } else {
        self.typeInfo.text =@"Global";
        [self.resetGeoButton setHidden:YES];
        [self.locationImage setHidden:YES];
        [self.locationLbl setHidden:YES];
    }
    locationText = @"Location";

    
    if([groupType containsString:@"private"]) {
        ...
        GName=getData[6];
        if (getData.count == 11 && [getData[9] length] > 0 && [getData[10] length]) {
            [self.locationLbl setTitle:[NSString stringWithFormat:@"%@, %@", getData[9], getData[10]] forState:(UIControlStateNormal)];
        }
        ...
        GName=getData[7];
        if (getData.count == 12 && [getData[10] length] > 0 && [getData[11] length]) {
            [self.locationLbl setTitle:[NSString stringWithFormat:@"%@, %@", getData[10], getData[11]] forState:(UIControlStateNormal)];
        }
...
-(void)plotData{
    ...
    self.groupDescTextView.text = groupDesc;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else  if(indexPath.section==2)
    {
        if ([groupType isEqualToString:@"private#local"]||[groupType isEqualToString:@"private#global"])
        {
            ...
            {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"There are no Group Join Request."   delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
...
//    
//    
//    if (textField == self.groupNameTextView) {
//        NSUInteger newLength = [self.groupNameTextView.text length] + [string length] - range.length;
//        return (newLength > 28) ? NO : YES;
//        
//    }
//    if (textField == self.groupDescTextView) {
//        NSUInteger newLength = [self.groupDescTextView.text length] + [string length] - range.length;
...
//        // update description
//       
//            groupDesc=self.groupDescTextView.text;
//            NSLog(@"1");
...
//      
//        
//    }else if(textField == self.groupNameTextView){
//        // update name
//        if ([self.groupNameTextView.text isEqualToString:@""]){
//            [HUD hide:YES];
//            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"Please enter group name"   delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
//            [alert show];
//            [self plotData];
//            
//        }else{
//            
//            if (![GName isEqualToString:self.groupNameTextView.text]){
//                
//                    NSLog(@"after edit name%@ old group name%@",self.groupNameTextView.text,groupName);
//                    NSString *postData= [NSString stringWithFormat:@"group_id=%@&group_name=%@",groupId,self.groupNameTextView.text];
//                    NSLog(@"$[%@]",postData);
//                    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if (connection == editGeoLocationConn) {
        [editGeoLocationResponse setLength:0];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    if (connection == editGeoLocationConn) {
        [editGeoLocationResponse appendData:data];
    }
    if (connection == deleteImageConn) {
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (connection != deleteImageConn){
        ...
        if (connection == updateNameConn) {
            ...
            NSLog(@"status = %@ error =  %@",status,error);
            if ([status isEqualToString:@"0"]){
                self.title=self.groupNameTextView.text;
                //groupName=nameTextField.text;
                NSString *query;
                if ([groupType isEqualToString:@"private#local"]||[groupType isEqualToString:@"private#global"] ||[groupType isEqualToString:@"private"]) {
                    query=[NSString stringWithFormat:@"update groups_private set group_name = '%@' where group_server_id = '%@'",[self.groupNameTextView.text normalizeDatabaseElement],groupId];
                }else{
                    query=[NSString stringWithFormat:@"update groups_public set group_name ='%@' where group_server_id = '%@'",[self.groupNameTextView.text normalizeDatabaseElement],groupId];
                }
        ...
        if (connection == editGeoLocationConn) {
            NSLog(@"====EVENTS");
            NSString *str = [[NSMutableString alloc] initWithData:editGeoLocationResponse encoding:NSASCIIStringEncoding];
            NSLog(@"Response:%@",str);
            SBJSON *jsonparser=[[SBJSON alloc]init];
            NSDictionary *res= [jsonparser objectWithString:str];
            NSLog(@" result %@",res);
            ...
            NSLog(@"status = %@ error =  %@",status,error);
            if ([status isEqualToString:@"0"]){
                [self.locationLbl setTitle:locationText forState:UIControlStateNormal];
                NSString *query;
                if ([groupType containsString:@"private"]) {
                    query=[NSString stringWithFormat:@"update groups_private set latitude ='%f', longitude ='%f', locality ='%@', country ='%@' where group_server_id = '%@'",latitude, longitude, locality, country, groupId];
                } else {
                    query=[NSString stringWithFormat:@"update groups_public set latitude ='%f', longitude ='%f', locality ='%@', country ='%@' where group_server_id = '%@'",latitude, longitude, locality, country, groupId];
                }
                NSLog(@"sub query %@",query);
                [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:query];
                [HUD hide:YES];
            ...
            [editGeoLocationConn cancel];
            editGeoLocationConn=nil;
        }
...
//    [self.groupDescTextView resignFirstResponder];
//    return YES;
//}
...
-(void)resetGroupLocationRequest
{
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
    NSString *postData = [NSString stringWithFormat:@"group_id=%@&latitude=%f&longitude=%f&locality=%@&country=%@",groupId,latitude,longitude,locality,country];
    NSLog(@"$[edit gps location %@]",postData);
    
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/scripts/edit_gps_group_location.php",gupappUrl]]];
    ...
    editGeoLocationConn = [[NSURLConnection alloc] initWithRequest:request delegate:self startImmediately:NO];
    
    [editGeoLocationConn scheduleInRunLoop:[NSRunLoop mainRunLoop] forMode:NSDefaultRunLoopMode];
    
    [editGeoLocationConn start];
    
    editGeoLocationResponse = [[NSMutableData alloc] init];
}
...
-(void)fetchLocation
{
    ...
    [[self appDelegate] getLocality:@"We need to access your current location in order to reset your group's location. Please enable access using the Settings option."];
}
...
- (IBAction)manageMembers:(id)sender {
    ...
    [self openGroupMember];
}

- (IBAction)changeGeoLocation:(id)sender {
...
#pragma mark - fetchLocationDelegate
- (void) didGetLocality:(NSDictionary *)localityDict {
    ...
    [self resetGroupLocationRequest];
}
