    ...
    if ([fileManager fileExistsAtPath: path]){
        ...
        if (![[data objectForKey:@"CreateGroup"] boolValue]) {
            ...
            if (screenWidth == IPHONE5_SCREEN_WIDTH) {
                backimage = [UIImage imageNamed:@"topic"];
            } else if (screenWidth == IPHONE6_SCREEN_WIDTH) {
                backimage = [UIImage imageNamed:@"topic-667"];
            } else if (screenWidth == IPHONE6P_SCREEN_WIDTH) {
                backimage = [UIImage imageNamed:@"topic-736"];
            } else {
                backimage = [UIImage imageNamed:@"topic"];
            }
            [Back setImage:backimage];
            [Back setImage:[backimage resizableImageWithCapInsets:UIEdgeInsetsMake(51, 0, 0, 28)]];
...
-(void)viewDidLoad{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshView) name:@"reloadPostList" object:nil];
    ...
    pullRfrs = [[UIRefreshControl alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 60)];
    ...
    newPostNotificationView.backgroundColor = [UIColor colorWithRed:1.0 green:207.0/255.0 blue:13.0/255.0 alpha:1.0];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if(![xmppRoom isJoined])
        [xmppRoom joinRoomUsingNickname:[NSString stringWithFormat:@"user_%@",[self appDelegate].myUserID] history:nil];
    postListData = [[NSMutableArray alloc] init];
    ...
    if (postListData.count > 0) {
        ...
        [self performSelectorInBackground:@selector(fetchLatestPost) withObject:nil];
    } else {
        [self refreshView];
    }
...
-(void)viewWillDisappear:(BOOL)animated{
    NSArray *private =  [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:
                         [NSString stringWithFormat:@"SELECT * from groups_private WHERE group_server_id = %@",self.groupId]];
    NSArray *public =  [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:
                        [NSString stringWithFormat:@"SELECT * from groups_public WHERE group_server_id = %@",self.groupId]];
    if (private.count > 0) {
        [[DatabaseManager getSharedInstance]executeQueryWithQuery:[NSString stringWithFormat: @"UPDATE groups_private SET new_post = 0 WHERE group_server_id=%@",
                                                                   self.groupId]];
    } else if (public.count > 0) {
        [[DatabaseManager getSharedInstance]executeQueryWithQuery:[NSString stringWithFormat: @"UPDATE groups_public SET new_post = 0 WHERE group_server_id=%@",
                                                                   self.groupId]];
    }
...
#pragma - mark
#pragma - mark APIRequest for fetching posts
...
-(void)UpdateDataBase:(NSArray*)JSON{
    ...
    if(latest){
        ...
        for (NSDictionary *dic in JSON) {
            BOOL update = NO;
            for (int n  = 0; n < postListData.count; n++) {
                NSMutableDictionary* postData = [postListData objectAtIndex:n];
                if ([[postData objectForKey:@"post_id"] isEqualToString:[dic objectForKey:@"post_id"]]) {
                    if ([[postData objectForKey:@"is_fav"] isEqualToString:@"1"]
                        && [[postData objectForKey:@"time"] intValue] < [[dic objectForKey:@"updated"] intValue])
                    {
                        ...
                        [postListData replaceObjectAtIndex:n withObject:postData];
                    }
                    update = YES;
                    break;
                }
        ...
        if(loadNew){
            [arr addObjectsFromArray:postListData];
            postListData = [arr mutableCopy];
            loadNew=false;
        }else {
            [postListData addObjectsFromArray:arr];
        }
        
        NSSortDescriptor *descriptor = [[NSSortDescriptor alloc] initWithKey:@"time"  ascending:NO];
        NSMutableArray *sortedArray=[[postListData sortedArrayUsingDescriptors:[NSArray arrayWithObjects:descriptor,nil]] mutableCopy];
        postListData = [sortedArray mutableCopy];
        [self checkNewComments];
        [self.postTable reloadData];
    }
}
...
-(void) checkNewComments {
    for (int n = 0; n < postListData.count; n++) {
        NSMutableDictionary* post = [postListData objectAtIndex:n];
        NSArray *unreadComment =  [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:
                                   [NSString stringWithFormat:@"SELECT read from chat_group WHERE post_id = %@ AND read = 0",[post objectForKey:@"post_id"]]];
        if (unreadComment && unreadComment.count > 0) {
            [post setValue:@"1" forKey:@"new_comment"];
        } else {
            [post setValue:@"0" forKey:@"new_comment"];
        }
        
        [postListData replaceObjectAtIndex:n withObject:post];
    }
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==postListData.count) {
        ...
        if (cell == nil) {
            ...
            indicator.center=CGPointMake(tableView.center.x, 33);
        ...
        if (requestFlag) {
            requestFlag=FALSE;
            [self performSelectorInBackground:@selector(fetchPreviousPost) withObject:nil];
        }
        return cell;
    }else{
        BOOL isContains=NO;
...
#pragma - mark
#pragma - mark PostListCellDelegate
...
//            [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:updateTime];
        ...
        if(total == 0)
            ...
            likelbl.userInteractionEnabled = YES;
        likelbl.text = [NSString stringWithFormat:@"%d Likes",total];
        NSString *query=[NSString stringWithFormat:@"UPDATE Post SET is_like=1,total_likes = %d ,is_fav = 1 WHERE post_id = %@",total,[groupData objectForKey:@"post_id"]];
        [[DatabaseManager getSharedInstance]executeQueryWithQuery:query];
...
//        [[DatabaseManager getSharedInstance]executeQueryWithQuery:query1];
    ...
    NSArray *updated = [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:[NSString stringWithFormat:@"SELECT updated FROM Post WHERE post_id = %@",[groupData objectForKey:@"post_id"]]];

    NSString* query;
    if (updated.count > 0) {
        query=[NSString stringWithFormat:@"insert into offlinelike (postid, groupid, likestatus, updatedTime) values (%@,%@,'%@',%@)",[groupData objectForKey:@"post_id"],self.groupId,likestatus,[[updated objectAtIndex:0] objectForKey:@"UPDATED"]];
    } else {
        query=[NSString stringWithFormat:@"insert into offlinelike (postid, groupid, likestatus, updatedTime) values (%@,%@,'%@',%@)",[groupData objectForKey:@"post_id"],self.groupId,likestatus,[NSString stringWithFormat:@"%.0f",time1]];
    }
    [[DatabaseManager getSharedInstance]saveDataInTableWithQuery:query];
    ...
    NSXMLElement *updatedTime;

    if (updated.count > 0) {
        updatedTime = [NSXMLElement elementWithName:@"updatedTime" stringValue:[[updated objectAtIndex:0] objectForKey:@"UPDATED"]];
    } else {
        updatedTime = [NSXMLElement elementWithName:@"updatedTime" stringValue:[NSString stringWithFormat:@"%.0f",time1]];
    }
...
-(void)openUserProfileImage:(UIImageView*)userImage{
    ...
    viewContact.userName = [groupData objectForKey:@"user_name"];
    [self.navigationController pushViewController:viewContact animated:YES];
    
}

#pragma - mark
#pragma - mark Report Post as Inappropriate
...
#pragma - mark
#pragma - mark Handlers for UIGestureRecognizer
...
-(void)commentAction:(NSNotification*)notification{
    ...
    for (NSDictionary *dic in postListData) {
        if([[dic objectForKey:@"post_id"] intValue] == [[notificationdata objectForKey:@"postid"] intValue]){
            ...
            NSArray *unreadComment =  [[DatabaseManager getSharedInstance]retrieveDataFromTableWithQuery:
                                       [NSString stringWithFormat:@"SELECT read from chat_group WHERE post_id = %@ AND read = 0",[mutDic objectForKey:@"post_id"]]];
            if (unreadComment && unreadComment.count > 0) {
                [mutDic setValue:@"1" forKey:@"new_comment"];
            } else {
                [mutDic setValue:@"0" forKey:@"new_comment"];
            }
            [_postTable beginUpdates];
            NSMutableArray *tempArray = [postListData mutableCopy];
            [tempArray replaceObjectAtIndex:[postListData indexOfObject:dic] withObject:mutDic];
            ...
            label.text = [NSString stringWithFormat:@"%d Gups",j];
            
            UIButton *comment = (UIButton*)[pCell viewWithTag:3];
            if (unreadComment && unreadComment.count > 0) {
                [comment setBackgroundImage:[UIImage imageNamed:@"pnewcomment"] forState:UIControlStateNormal];
            }
            [_postTable endUpdates];
        }
        i++;
...
#pragma - mark
#pragma - mark Selector for create post action
...
-(CGSize)calculateHeight:(NSString*)data{
    ...
    CGFloat width = 280;
    UIFont *font = [UIFont fontWithName:@"Helvetica" size:12.0f + [[self appDelegate] increaseFont]];
...
-(NSString*)newLineStringFormat:(NSArray*)atrArray{
    ...
    for (NSString *first in atrArray) {
        int l = first.length / perLine + 1;
        if (lines + l >= maxLine) {
            l = maxLine - lines;
            int length = (perLine * l > first.length)? (perLine * l): first.length;
            int last = length - perLine * (l-1);
            if (last > 40) {
                last = 40;
            }
            length = perLine*(l-1) + last;
            cText = [NSString stringWithFormat:@"%@\n%@...   %@",cText,[first substringWithRange:NSMakeRange(0, (first.length<length)?first.length:length)],@"More"];
            break;
        } else {
            lines += l;
            cText = [NSString stringWithFormat:@"%@\n%@",cText,first];
