...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        messageText = [[TTTAttributedLabel alloc] initWithFrame: CGRectZero];
        messageText.opaque = YES;
        messageText.clearsContextBeforeDrawing = NO;
...
- (void)drawCell:(NSDictionary*)data withIndexPath:(NSIndexPath*)indexPath{
    ...
    if([filemgr fileExistsAtPath:Filepath] == YES){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSData *imgData = [NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:
                                                                                  @"%@/scripts/media/images/profile_pics/%@", gupappUrl, [data objectForKey:@"SENDER_IMAGE"]]]];
    ...
    if([[data objectForKey:@"MESSAGE_TYPE"] isEqual:@"image"]){
        ...
        if([filemgr fileExistsAtPath:Filepath] == YES){
            ...
            [postImage setBackgroundColor:[UIColor lightGrayColor]];
            UIActivityIndicatorView* indicator = [[UIActivityIndicatorView alloc] initWithFrame:
                                                  CGRectMake(CGRectGetWidth(postImage.frame)/2 - 10, CGRectGetHeight(postImage.frame)/2 - 10, 20, 20)];
            [postImage addSubview:indicator];
            [indicator startAnimating];
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                NSData *imgData = [NSData dataWithContentsOfURL: [NSURL URLWithString: [NSString stringWithFormat: @"%@/scripts/media/images/chat_files/%@",
                                                                                        gupappUrl, [data objectForKey:@"MESSAGE_FILENAME"]]]];
                dispatch_async(dispatch_get_main_queue(), ^{
