...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        username.tag = 100;
        username.font = [UIFont fontWithName:@"Dosis-SemiBold" size:15.0f + addFont];
        ...
        self.lastUpdatedTime.font = [UIFont fontWithName:@"Dosis-Regular" size:9.0f + addFont];
...
- (void)drawCell:(NSDictionary*)data{
    ...
    if ([[data objectForKey:@"imageCount"] intValue]==0) {
        ...
        _commentLbl.text = [NSString stringWithFormat:@"%@ Gups",totalComments];
        ...
        if ([[self reuseIdentifier] isEqualToString:@"loadMoreCell"]) {
            ...
            NSRange range = [post_desc.text rangeOfString:@"Less" options:NSBackwardsSearch];
            ...
            if (size.height > 75.0) {
                ...
                if(newLineStrArray.count>5){
                    ...
                    NSRange range = [post_desc.text rangeOfString:@"More" options:NSBackwardsSearch];
        ...
        if ([[data objectForKey:@"new_comment"] isEqualToString:@"1"]) {
            [commentPost setBackgroundImage:[UIImage imageNamed:@"pnewcomment"] forState:UIControlStateNormal];
        } else {
            [commentPost setBackgroundImage:[UIImage imageNamed:@"pcomment"] forState:UIControlStateNormal];
        }
    }else{
        ...
        _commentLbl.text = [NSString stringWithFormat:@"%@ Gups",[data objectForKey:@"total_comments"]];
        ...
        if ([[self reuseIdentifier] isEqualToString:@"loadMoreCell"]) {
            ...
            cText=[NSString stringWithFormat:@"%@   Less",disString];
            post_desc.text= cText;
            NSRange range = [post_desc.text rangeOfString:@"Less" options:NSBackwardsSearch];
            [post_desc addLinkToURL:[NSURL URLWithString:@"action://readless"] withRange:range];
            
            CGSize size = [self calculateHeight:cText];
        ...
        _spinner.center = CGPointMake(post_image.frame.size.width/2, post_image.frame.size.height/2);
...
-(void)leftSweepAction:(UIGestureRecognizer*)gesture{
    ...
    [self.cellDelegate leftImageAction:image];
    
}

-(void)rightSweepAction:(UIGestureRecognizer*)gesture{
    UIImageView *image = (UIImageView*)[gesture view];
    [self.cellDelegate rightImageAction:image];
...
- (void)attributedLabel:(TTTAttributedLabel *)label didSelectLinkWithURL:(NSURL *)url{
    if ([[url scheme] hasPrefix:@"action"]) {
        if ([[url host] hasPrefix:@"readmore"]) {
            [self.cellDelegate cellHeightChange:label];
        }else if ([[url host] hasPrefix:@"readless"]){
            [self.cellDelegate readLessAction:label];
        }
    }else{
...
-(CGSize)calculateHeight:(NSString*)data{
    ...
    CGFloat width = 280;
    UIFont *font = [UIFont fontWithName:@"Helvetica" size:12.0f+[(AppDelegate*)[[UIApplication sharedApplication] delegate] increaseFont]];
...
-(void)commentBtnAction:(UIButton*)btn{
    [self.cellDelegate commentButtonClick:btn];
}

-(void)shareBtnAction:(UIButton*)btn{
    [self.cellDelegate sharePost:btn];
}

-(void)likeButtonAction:(UIButton*)btn{
    [self.cellDelegate likePost:btn];
}

-(void)likeLblAction:(UIGestureRecognizer*)gesture{
    [self.cellDelegate likeLabelAction:gesture];
    
}
-(void)commentLblAction:(UIGestureRecognizer*)gesture{
    [self.cellDelegate commentLblAction:gesture];
}
-(void)repostBtnAction:(UIButton*)btn{
    [self.cellDelegate repostPost:btn];
}

-(void)openProfile:(UIGestureRecognizer*)gesture{
    
    [self.cellDelegate openUserProfileImage:(UIImageView*)gesture.view];
}
...
-(void)openImage:(UIGestureRecognizer*)gesture{
    ...
    [self.cellDelegate openImageInFullSize:(UIImageView*)[gesture view] currentImageUrl:postImagePath];
}
